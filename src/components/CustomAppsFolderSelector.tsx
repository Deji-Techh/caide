...
export function CustomAppsFolderSelector() {
  ...
  const handleResetToDefault = async () => {
    try {
      ...
      await fetchCustomAppsFolder();
      showSuccess("CAIDE apps folder reset successfully");
    } catch (error: any) {
      showError(`Failed to reset CAIDE apps folder path: ${error.message}`);
    }
  };
  ...
  const fetchCustomAppsFolder = async () => {
    try {
      ...
      showError(`Failed to fetch CAIDE apps folder path: ${error.message}`);
    }
  };
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400">
          <p>
            {isPathAvailable
              ? "This is the top-level folder where CAIDE stores new applications."
